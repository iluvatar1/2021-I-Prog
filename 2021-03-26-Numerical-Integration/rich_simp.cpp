...
double richardson_simpson(double a, double b, int npart);
...
int main(int argc, char *argv[]) {
    ...
    for (int p = 0; p <= 6; p++){
        ...
        double delta_rich_simp = std::fabs(richardson_simpson(A, B, in)-exact)/exact;
        std::cout << in << "\t"
                  << delta_trapecio << "\t"
                  << delta_rich_trap << "\t"
                  << delta_simpson << "\t"
...
double richardson_simpson(double a, double b, int npart)
{
    ...
    double i1 = simpson(a, b, npart);
    double i2 = simpson(a, b, npart*2);
    double integral = i2 + (i2-i1)/(std::pow(2, alpha) - 1);
    return integral;
}

double richardson_simpson(double a, double b, int npart, fptr alg, int alpha)
{
